...
void UDMSGameInstance::Init()
{
    ...
    if (IOnlineSubsystem::Get()->GetSubsystemName() != "NULL")  DefaultSessionSettings.bIsLANMatch=false;
    else    DefaultSessionSettings.bIsLANMatch=true;
...
void UDMSGameInstance::OnFindSessionsComplete(bool Succeeded)
{
    ...
    DMS_LOG_SIMPLE(TEXT("OnFindSession Complete %d / %d"),Results[0].Session.NumOpenPrivateConnections,Results[0].Session.SessionSettings.NumPublicConnections);
...
void UDMSGameInstance::JoinDMSSession(FName SessionName)
{
    ...
    if (IOnlineSubsystem::Get()->GetSubsystemName() != "NULL")  SessionSearch->bIsLanQuery = true;
    else    SessionSearch->bIsLanQuery = false;
    SessionSearch->MaxSearchResults = 100;
    SessionSearch->QuerySettings.Set(SEARCH_PRESENCE, true, EOnlineComparisonOp::Equals);
       
    //auto OnFindCompLambda = [=](bool Succeeded){
    //    if(!Succeeded) return;
    //    TArray<FOnlineSessionSearchResult> Results = SessionSearch->SearchResults;

    //    if(Results.Num()==0) return;

    //    SessionInterface->JoinSession(0, SessionName, Results[0]);
    //    SessionInterface->OnFindSessionsCompleteDelegates.Remove(this->OnFindDelegateHandle);
    //};

    //OnFindDelegateHandle = SessionInterface->OnFindSessionsCompleteDelegates.AddLambda(OnFindCompLambda);
