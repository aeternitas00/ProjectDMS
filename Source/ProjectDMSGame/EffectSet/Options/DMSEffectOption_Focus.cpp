...
void UDMSEffectOption_Focus::OnExecuteOption_Implementation(UDMSSequence* iSourceSeq, UDMSEffectInstance* iEI, const FOnOptionCompleted& OnOptionCompleted)
{
	DMS_LOG_SIMPLE(TEXT("FOCUS"));
	AActor* Target = Cast<AActor>(FocusTarget->GetTargets(iEI->GetOuter(), iSourceSeq)[0]);
